...
const getGuruJabatan = async (req, res) => {
  const { userId } = req.token;
  if (userId) {
    const payload = req.query;
    const validatePayload = validator.isValidPayload(payload, queryModel.getGuruId);
    const getRequest = async (result) => {
      ...
      return queryHandler.getGuruJabatan(result.data);
    };
...
const getTenagaAhliJabatan = async (req, res) => {
  const { userId } = req.token;
  if (userId) {
    const payload = req.query;
    const validatePayload = validator.isValidPayload(payload, queryModel.getTenagaAhliId);
    const getRequest = async (result) => {
      ...
      return queryHandler.getTenagaAhliJabatan(result.data);
    };
...
const getGuruOneJabatan = async (req, res) => {
  const { userId } = req.token;
  if (userId) {
    const payload = req.params;
    ...
    const getRequest = async (result) => {
      ...
      return queryHandler.getGuruOneJabatan(result.data);
    };
...
const exportSiswa = async (req, res) => {
  ...
  if (userId) {
    const payload = req.body;
    const validatePayload = validator.isValidPayload(payload, commandModel.exportSiswa);
    const postRequest = async (result) => {
      if (result.err) {
        return result;
      }
      return commandHandler.exportSiswa(result.data);
    };
    const sendResponse = async (result) => {
      /* eslint no-unused-expressions: [2, { allowTernary: true }] */
      (result.err) ? wrapper.response(res, 'fail', result, 'Export Siswa')
        : wrapper.responseBuffer(res, result);
      //local
      // : wrapper.response(res, 'success', result, 'export siswa', http.CREATED);
    };
...
const exportDataSiswa = async (req, res) => {
  ...
  if (userId) {
    const payload = req.body;
    const validatePayload = validator.isValidPayload(payload, commandModel.exportDataSiswa);
    const postRequest = async (result) => {
      if (result.err) {
        return result;
      }
      return commandHandler.exportDataSiswa(result.data);
    };
    const sendResponse = async (result) => {
      /* eslint no-unused-expressions: [2, { allowTernary: true }] */
      (result.err) ? wrapper.response(res, 'fail', result, 'Export Data Siswa')
        : wrapper.responseBuffer(res, result);
      //local
      // : wrapper.response(res, 'success', result, 'export siswa', http.CREATED);
    };
